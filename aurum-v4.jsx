    ...
    setTimeout(() => {
      try {
        if (mode === "register") {
          ...
          if (localStorage.getItem(key)) { setErr("Account already exists. Please sign in."); setLoading(false); return; }
          ...
          if (hash(pw + email.toLowerCase()) !== user.pwHash) { setErr("Incorrect password. Please try again."); setLoading(false); return; }
          setLoading(false);
          onLogin(user);
        }
      } catch (e) {
      ...
      <div style={{ width: "100%", maxWidth: 428 }}>
        ...
        <div className="auth-card">
          ...
          <div style={{ padding: "28px 32px 32px" }}>
            ...
            <button className="google-btn" onClick={() => setErr("Google login requires backend setup.")}>
...
export default function AurumPro() {
  ...
  const loadUserData = (u) => {
    try {
      ...
      const bd = localStorage.getItem(`aurum_balance_${u.id}`);
      if (td) setTrades(JSON.parse(td));
      if (sd) setStrats(JSON.parse(sd));
      if (bd) setStartBal(parseFloat(bd));
  ...
  const saveTrades = useCallback((t, u) => {
    if (!u) return;
    setSaving(true);
    try { localStorage.setItem(`aurum_trades_${u.id}`, JSON.stringify(t)); } catch (e) {}
    setTimeout(() => setSaving(false), 400);
  }, []);

  const saveStrats = useCallback((s, u) => {
    if (!u) return;
    try { localStorage.setItem(`aurum_strats_${u.id}`, JSON.stringify(s)); } catch (e) {}
  }, []);
  ...
  const confirmDelete = useCallback((id) => {
    ...
    if (!trashBin) return;
    updateTrades(p => p.filter(x => x.id !== id));
    ...
    showToast("Trade deleted", {
      ...
      onUndo: () => {
        updateTrades(p => {
          if (p.some(x => x.id === trashBin.id)) return p;
          return [...p, trashBin].sort((a, b) => b.date.localeCompare(a.date));
        });
        showToast("Trade restored ✓", { type: "success" });
      }
    });
  }, [trades, updateTrades, showToast]);
  ...
  const saveInlineEdit = () => {
    ...
    updateTrades(p => p.map(x => x.id === inlineEditId ? updated : x));
                  ...
                  <input
                    ...
                    onKeyDown={e => {
                      if (e.key === "Enter") {
                        ...
                        if (v > 0) {
                          setStartBal(v);
                          try { localStorage.setItem(`aurum_balance_${user?.id}`, String(v)); } catch {}
                          ...
                          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            <input
                              ...
                              onKeyDown={e => {
                                if (e.key === "Enter") {
                                  const v = parseFloat(startBalInput);
                                  if (v > 0) { setStartBal(v); try { localStorage.setItem(`aurum_balance_${user?.id}`, String(v)); } catch {} showToast("Balance updated ✓"); }
                                  setStartBalEdit(false);
                                }
                                if (e.key === "Escape") setStartBalEdit(false);
                              }}
